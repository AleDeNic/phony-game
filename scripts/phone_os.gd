...
func go_to_screen(screen: Control) -> void:
	reset_screens()
	screen.visible = true
	player_manager.set_phone_state(player_manager.PhoneState[screen.name.to_upper()])
	if player_manager.phone_state != player_manager.PhoneState.APPS:
...
func handle_battery() -> void:
	battery_timer.paused = not player_manager.is_battery_active()
...
func _on_back_pressed() -> void:
	if player_manager.phone_state == player_manager.PhoneState.ASUKACHAT:
		go_to_screen(chats)
	else:
		go_to_screen(apps)
